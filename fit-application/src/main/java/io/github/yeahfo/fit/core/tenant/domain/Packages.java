...
import static io.github.yeahfo.fit.core.plan.domain.Plan.FREE_PLAN;
import static io.github.yeahfo.fit.core.plan.domain.Plan.planFor;
import static io.github.yeahfo.fit.core.plan.domain.PlanType.FREE;
...
public class Packages {
    ...
    public void validateAddPlanDuration( int yearDuration ) {
        ...
        if ( expiredAt.isAfter( fiveYearsLater ) ) {
            throw new FitException( PACKAGE_DURATION_TOO_LONG, "套餐总时长不能超过" + MAX_PLAN_DURATION_YEARS + "年。" );
        }
    }

    public void validateAddExtraMembers( int amount ) {
        if ( this.extraMemberCount + amount > MAX_EXTRA_TENANT_MEMBERS ) {
            throw new FitException( MAX_TENANT_MEMBER_SIZE_REACHED, "增购成员总数最多不超过" + MAX_EXTRA_TENANT_MEMBERS + "名。" );
        }
    }

    public void validateAddExtraStorage( int amount ) {
        if ( this.extraStorage + amount > MAX_EXTRA_STORAGES ) {
            throw new FitException( MAX_EXTRA_STORAGE_REACHED, "增购存储空间总量不超过" + MAX_EXTRA_STORAGES + "G。" );
        }
    }

    public void validateAddExtraVideoTraffic( int amount ) {
        if ( this.extraRemainVideoTraffic + amount > MAX_EXTRA_VIDEO_TRAFFIC ) {
            throw new FitException( MAX_VIDEO_TRAFFIC_REACHED, "增购视频总流量不超过" + MAX_EXTRA_VIDEO_TRAFFIC + "G。" );
        }
    }

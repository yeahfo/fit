...
import static io.github.yeahfo.fit.common.utils.RegexConstants.EMAIL_PATTERN;
import static io.github.yeahfo.fit.common.utils.RegexConstants.MOBILE_PATTERN;
...
public interface CommonUtils {
    ...
    static Instant startOfLastWeek( ) {
        return LocalDate.now( )
                .minusWeeks( 1 )
                .with( MONDAY )
                .atStartOfDay( systemDefault( ) )
    ...
    static Instant startOfLastMonth( ) {
        return LocalDate.now( )
                .minusMonths( 1 )
                .with( firstDayOfMonth( ) )
                .atStartOfDay( systemDefault( ) )
    ...
    static Instant startOfCurrentSeason( ) {
        int currentMonth = LocalDate.now( ).getMonth( ).getValue( );
        int startMonth;
        if ( currentMonth <= 3 ) {
            startMonth = 1;
    ...
    static Instant startOfLastSeason( ) {
        int currentMonth = LocalDate.now( ).getMonth( ).getValue( );
    ...
    static Instant startOfCurrentYear( ) {
        return LocalDate.now( )
                .with( firstDayOfYear( ) )
                .atStartOfDay( systemDefault( ) )
                .toInstant( );
    ...
    static Instant startOfLastYear( ) {
        return LocalDate.now( )
                .minusYears( 1 )
                .with( firstDayOfYear( ) )
                .atStartOfDay( systemDefault( ) )
    ...
    static boolean isMobileNumber( String value ) {
    ...
    static String maskMobileOrEmail( String mobileOrEmail ) {
    ...
    static List< Double > splitAndSortNumberSegment( String str ) {
        if ( isBlank( str ) ) {
            return List.of( );
        }
    ...
    static String[] splitSearchBySpace( String search ) {
        if ( isBlank( search ) ) {
            return new String[ 0 ];
        }
